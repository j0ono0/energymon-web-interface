...
});"use strict";Vue.component("tab", { props: { tabIndex: { required: true }, name: { required: true }, isActive: { default: false } }, computed: { classes: function classes() {
      var classes = ["tabs__tab"];if (this.isActive) {
        classes.push("active");
      }return classes.join(" ");
    }, href: function href() {
      return "#" + this.tabIndex;
    } }, template: '\n        <a \n            :class="classes" \n            :href="href"\n            v-on:click.stop="$emit(\'updatetabs\',tabIndex)"\n        >\n            {{ name }}\n        </a>\n    ' });Vue.component("tabs", { props: { tabs: Array }, data: function data() {
    return { tabStatus: [] };
  }, methods: { updatetabs: function updatetabs(tabindex) {
      var newTabStatus = Array(this.$children.length);newTabStatus[tabindex] = true;this.tabStatus = newTabStatus;
    } }, mounted: function mounted() {
    this.updatetabs(0);
  }, template: '\n        <nav class="tabs" >\n            <tab \n                v-for="(tab,i) in tabs" \n                :key="i"\n                :name="tab.name" \n                :isActive="tabStatus[i]"\n                :tabIndex="i" \n                @updatetabs="updatetabs"\n            ></tab>\n        </nav>\n    ' });"use strict";Vue.component("wifi-list", { data: function data() {
    return { message: "Locating WiFi networks...", availableWiFi: [] };
  }, methods: { get_wifi: function get_wifi(url) {
      var httpRequest = new XMLHttpRequest();httpRequest.onreadystatechange = function () {
        if (httpRequest.readyState === XMLHttpRequest.DONE) {
          if (httpRequest.status === 200) {
            var response = JSON.parse(httpRequest.responseText);this.message = null;this.availableWiFi = response.data;
          } else {
            this.message = "There was communication problem whilst contacting your Energymon device. Reload this page to try again.";
          }
        }
      }.bind(this);httpRequest.open("GET", url);httpRequest.send();
    } }, created: function created() {
    this.get_wifi("/energymon/json/wifi");
  }, template: '\n        <div class="wifilist">\n            <p v-if="message != null">{{ message }}</p>\n            <ul>\n                <li v-for="network in availableWiFi">\n                    <p>{{ network.name }} ({{ network.strength }})</p>\n                </li>\n            </ul>\n        </div>\n    ' });"use strict";Vue.component("alert", { props: { message: [String] }, template: '\n        <div class="alert">\n            <p>{{ message }}</p>\n        </div>\n    ' });var app = new Vue({ el: "#application", data: { message: "Replace this message!" }, methods: {}, created: function created() {} });